...
const FormEditOnly = ({
  ...
  console.log('day la dattaHandle', attachmentsData);
  ...
  useEffect(() => {
    ...
    if (attachmentsData) {
      // setDefaultUpload(attachmentsData);
    }
  ...
  const props = {
    ...
    defaultFileList: attachmentsData,
    ...
    onChange(info) {
      ...
      if (info.file.status === 'done') {
        if (info.fileList.length > 0) {
          const getFileList = info.fileList.map((item) => {
            console.log('day la itemFileList', item);
            return {
      ...
      if (info.file.status === 'removed') {
        ...
        const convertJSON = JSON.stringify(fileList);
        form.setFieldsValue({ attachments: convertJSON });
        if (getValueUpload) {
          getValueUpload(fileList, nameForm);
        }
      }
    }
  };
